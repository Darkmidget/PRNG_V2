...
6. **Top module naming:**
   - Update `scripts/build_tools/build.tcl` to set correct top module
...
### Updating Top Module
Edit `scripts/build_tools/build.tcl`:
```tcl
set_property top YOUR_MODULE_NAME [current_fileset]
```

Edit `scripts/build_tools/build.tcl` and `scripts/build_tools/program.tcl`:
```tcl
